---
reviewed: true
source: hand-written (edited 2026-07-06)
---

Housing, Infrastructure and Communities Canada (HICC) is responsible for federal policies and programs that support public infrastructure, affordable housing, and community development. Working with provinces, territories, and municipalities, it funds major infrastructure projects and administers the National Housing Strategy, which supports the construction, repair, and preservation of affordable housing. It also oversees the Canada Mortgage and Housing Corporation (CMHC) and supports public transit, clean energy, and community infrastructure through programs such as the Canada Infrastructure Bank.

HICC spent {{totalSpending}} in fiscal year (FY) 2024, or {{percentageOfFederal}} of the $513.9 billion in overall federal spending. Founded in 2005 as the Office of Infrastructure of Canada, the department's mandate and budget have expanded substantially, and its share of federal spending has roughly doubled over the past decade.

Spending is dominated by two entities: the Office of Infrastructure of Canada ($7.6 billion) and the Canada Mortgage and Housing Corporation ($5.5 billion), followed by the Windsor-Detroit Bridge Authority ($1.3 billion).

Much of this spending reaches provinces, territories, and municipalities through infrastructure and housing programs. The largest FY 2024 transfers were the Canada Community-Building Fund ($2.4 billion), the Investing in Canada Infrastructure Program ($2.0 billion), and the New Building Canada Fund ($1.0 billion), along with funding to alleviate homelessness and to support disaster mitigation and adaptation.

HICC is led by the [Minister of Housing, Infrastructure and Communities](https://www.pm.gc.ca/en/cabinet/honourable-nate-erskine-smith), who is appointed by the Governor General on the advice of the Prime Minister and sworn into office at Rideau Hall as a member of the King's Privy Council for Canada. The minister is one of the [cabinet members](https://www.pm.gc.ca/en/cabinet) who serve at the Prime Minister's discretion, remaining in the role until a successor is sworn in.

<!--
Figures: EN spending para uses {{totalSpending}} ($14.56B) and {{percentageOfFederal}}
(2.8%), matching the JSON (vol2_appropriations) and charts. FR literal "14,5 milliards"
replaced by token. Removed the original page's misuse of {{totalSpending}} mid-sentence.
"$513.9 billion" retained (matches summary.json). Growth narrative kept qualitatively —
supported by historicalShare (1.37% in 2014 → 2.83% in 2024).
Restored from dropped entity list, verified against JSON: Office of Infrastructure of
Canada $7.6B, CMHC $5.5B, Windsor-Detroit Bridge Authority $1.3B. Transfers verified
against transferPayments: Canada Community-Building Fund $2.4B, Investing in Canada
Infrastructure Program $2.0B, New Building Canada Fund $1.0B, homelessness, disaster
mitigation.
Dropped "ranked eighth", "10 departments = 73.2%", the "$250,000" 2005 startup figure
(unverifiable), and the pandemic totals ($410.2B/$420B/$720.3B overall and "$5.9B in
2019" — old basis, unverifiable). Dropped generic direct/indirect boilerplate.
-->
